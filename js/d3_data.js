...
var d3data = {
    ...
    prepareData: function (uri, lang, endpoint, project, afterInit, expandTo) {
        let query = `PREFIX skos: <http://www.w3.org/2004/02/skos/core#>
            ...
            SELECT DISTINCT (COALESCE(?sC, '') AS ?sColor) (COALESCE(?sL, STRAFTER(STR(?s), "/")) AS ?sLabel)
            ?s ?x ?o
            ...
            WHERE { GRAPH ?g {
                ...
                ?s skos:narrower ?o .
                # Keep track of the property path (for labeling purposes)
                BIND(skos:narrower AS ?x)
                # Optional label and metadata
                OPTIONAL { ?s skos:prefLabel ?sL FILTER (lang(?sL) = 'de') }
                OPTIONAL { ?o skos:prefLabel ?oL FILTER (lang(?oL) = 'de') }
